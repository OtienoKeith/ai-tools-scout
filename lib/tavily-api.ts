...
Return ONLY actual software tools, not articles, not reviews, not listicles, not blog posts. Format as a clear, structured list.`;
    ...
    // If no good match found in our database, return a "no results" response
    // instead of showing inaccurate results that damage credibility
    const noResultsResponse: TavilyTool[] = [
      {
        id: "no-results",
        name: "No AI tools found",
        description: `Sorry, we couldn't find specific AI tools for "${query}". Try searching for more common categories like "writing", "design", "project management", "marketing", or "coding".`,
        pricing: "N/A",
        url: "#",
    ...
    searchCache.set(cacheKey, { tools: noResultsResponse, timestamp: Date.now() });
    return noResultsResponse;
  } catch (error) {
    console.error("Search error:", error);
    // Return "no results" on error instead of fallback tools
    return [
      {
        id: "error",
        name: "No AI tools found",
        description: "Sorry, we couldn't find any AI tools for your search. Please try a more common category like 'writing', 'design', or 'project management'.",
        pricing: "N/A",
        url: "#",
        pricingUrl: undefined
      }
